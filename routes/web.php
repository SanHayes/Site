...
Route::get('/api/custorm', function (Request $request) {
    ...
    if($terminal=='mobile')
    {
        return ['mobile' => $c2, 'whatsapp' => $c3];
    }
    return ['pc' => $c2, 'whatsapp' => $c3];
});
Route::get('/api/web', function (Request $request) {
    $web['name'] = Setting::getValueByKey('web_name', '');
    $web['logo'] = Setting::getValueByKey('web_logo', '');
    $web['onlinereq'] = Setting::getValueByKey('online_req', 0);
    $web['invitation_code'] = Setting::getValueByKey('invitation_code', 0);
    return $web;
});
Route::get('/app/download', function (Request $request) {
    // $data['android_direct'] = Setting::getValueByKey('android_direct');
    // $data['ios_direct'] = Setting::getValueByKey('ios_direct');
    $data['app_direct'] = Setting::getValueByKey('app_direct');
    return $data;
});
Route::get('/mobile/download.html', function (Request $request) {
    if (strpos($_SERVER['HTTP_USER_AGENT'], 'iPhone') || strpos($_SERVER['HTTP_USER_AGENT'], 'iPad')) {
        return redirect(Setting::getValueByKey('app_direct'));
...
Route::group(['middleware' => ['lang', /*'check_user'*/]], function () {
    ...
    Route::get('/login', function () {
        ...
        session()->put('admin_secret', '');
        session()->put('admin_qrcod_url', '');
        $google_verify = Setting::getValueByKey('google_verify', '');
        return view('admin/login',['google_verify' => $google_verify]);
    });
    ...
    Route::get('/api/trans', function (Request $request) {
        ...
        $from = $request->input('from','zh');
        $to = $request->input('to','en');
        $val = $request->input('val','百度翻译');
...
//<--LDH-->
    ...
    Route::get('/api/wallet/notify', 'Api\WalletController@notify');
    ...
    Route::get('api/currency/type', 'Api\CurrencyController@type');//币种类型列表
...
//Route::any('api/currency/new_timeshar', 'Api\CurrencyController@newTimeshars')->middleware(['cross']); //K线分时数据，对接tradeingview
    ...
    Route::get('order_voice_list', 'Api\VoiceController@lists');//获取充值提现订单未查看数据
//<--LDHend-->
...
//******************************api接口需要登录的**********************
    Route::group(['prefix' => 'api', 'middleware' => ['check_api', /*'check_user'*/]], function () {
        ...
        Route::post('set_password', 'Api\Qrpng@forgetPassword');//设置登录密码
        ...
        Route::post('wallet/online_req', 'Api\WalletController@onlineReq');
...
//管理后台
    Route::group(['prefix' => 'admin', 'middleware' => ['admin_auth']], function () {
        ...
        Route::post('app/save', function (Request $request) {
            ...
            $save_path = dirname(__DIR__) . '/public/download';

            $ifile = $_FILES['ifile'] ?? false;

            if (is_array($ifile)) {
                rename($ifile['tmp_name'], $save_path . '/jpx.mobileconfig');
            }

//            var_dump(IA_ROOT);
            $afile = $_FILES['afile'] ?? false;
            if (is_array($afile)) {
                rename($afile['tmp_name'], $save_path . '/jpx.apk');
//                rename($afile['tmp_'])
            }
        ...
        Route::get('/google/setting', 'Admin\GoogleController@setting');//谷歌验证码
        ...
        Route::post('/manager/google_verify', 'Admin\AdminController@google_verify');//修改谷歌验证码
        ...
        Route::get('news_list', 'Admin\NewsController@list');
        ...
        //币种类型
        Route::get('currency_type', 'Admin\CurrencyController@type');//币种类型
        Route::get('currency_type_list', 'Admin\CurrencyController@typeList');//币种类型
        Route::get('currency_type_add', 'Admin\CurrencyController@typeAdd')->middleware(['demo_limit']);//添加币种类型
        Route::post('currency_type_postAdd', 'Admin\CurrencyController@postTypeAdd')->middleware(['demo_limit']);//添加币种类型
        Route::post('currency_type_del', 'Admin\CurrencyController@typeDelete')->middleware(['demo_limit']);//添加币种类型
