...
use App\{Currency, CurrencyMatch, CurrencyQuotation, MarketHour, MyQuotation, Needle, Setting, UserChat, UsersWallet};
...
//            $this->writeRedisTimer = Timer::add($this->micro_trade_freq, [$this, 'writeRedis'], [], true);
            ...
            $this->iepnDepthTimer = Timer::add($this->micro_trade_freq, [$this, 'sendDepthAndDetail'], [], true);
        }
...
//                $con->send($sub_data);
    ...
    public function onMarketKline()
    {
        foreach ($this->currencys as $currency) {
            ...
            //file_put_contents('/www/wwwroot/Site/app/Utils/Workerman/kline_data.txt',json_encode($kline_data));
    ...
    public function handleMicroTrade()
    {
        ...
        if(count($futures_market_buy) > 0){
            foreach ($futures_market_buy as $key => $item){
                DB::beginTransaction();
                DB::table('futures_market_buy')->where(['id'=>$item->id])->update(['state'=>2]);
                $wallet = UsersWallet::where('user_id', $item->user_id)->where('currency', 3)->lockForUpdate()->first();
                $res = change_wallet_balance($wallet, 4, $item->money + $item->interest, 666, '理财产品分利，返还本息USDT:' . ($item->money + $item->interest));
                if ($res) {
                    DB::commit();
                } else {
                    DB::rollBack();
        ...
        foreach ($market_data as $period => $data) {
            foreach ($data as $key => $symbol) {
                if ($period == '1min') {
                    ...
                    {
                        echo '秒合约时间:' . time() . ', Symbol:' . $key . '.' . $period . '数据:' . $symbol['kline_data']['close'] . PHP_EOL;
