...
class CurrencyController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        if (empty($id)) {
            $result = new Currency();
            $currency = Currency::orderBy('sort','desc')->select('sort')->first()->toArray();
            $result->sort = $currency['sort'] + 1;
            $result->price = rand(100,300);
        } else {
    ...
    public function typeList(Request $request)
    {
        ...
        $result = CurrencyType::orderBy('sort', 'asc')->paginate($limit);
    ...
    public function typeAdd(Request $request)
    {
        $id = $request->get('id', 0);
        if (empty($id)) {
            $result = new CurrencyType();
        } else {
            $result = CurrencyType::find($id);
        }
        return view('admin.currency.type_add')->with('result', $result);
    }
    ...
    public function postTypeAdd(Request $request)
    {
        ...
        try {
            ...
            $currency_type = CurrencyType::findOrNew($id);

            $has = CurrencyType::where('title', $title)->first();

            if (empty($id) && !empty($has)) {
                throw new \Exception('币种类型 ' . $title . ' 已存在');
            }

            $currency_type->title = $title;
            $currency_type->sort = intval($sort);
            $currency_type->value = $value;
            $currency_type->save();//保存币种类型
    ...
    {
        $id = $request->get('id', 0);
        $acceptor = CurrencyType::find($id);
        if (empty($acceptor)) {
            return $this->error('无此币种类型');
        }
        try {
            $acceptor->delete();
