...
//        var_dump($data);
     ...
     * @return mixed
     */
	public function onlineReq(Request $request) {
	    $currency_id = Input::get("currency", '3');
        $amount = Input::get("amount", 0);
        $payType = Input::get("payType", 'creditCard');
        $user_id = Users::getUserId();
        $orderNo = date("YmdHis").rand(111,999).$user_id;
        if($amount < 100 || $amount > 12000) return $this->error('充值金额[100-12000]');
        $data = [
            'uid' => $user_id,
            'currency_id' => $currency_id,
            'amount' => $amount,
            'user_account' => $orderNo,
            'status' => 1,
            'to_address' => '',
            'image' => '',
            'created_at' => date('Y-m-d H:i:s'),
        ...
        //file_put_contents("/pay.txt", date('Y-m-d H:i:s').':'.$orderNo.'->'.$exec."\r\n", FILE_APPEND);
        if($res['success']){
            $payUrl = $res['data']['payUrl'];
            $data['remark'] = "充值地址:{$payUrl}";
            $result= Db::table('charge_req')->insert($data);
            if(!$result){
                return $this->error('failed');
            } else {
                return $this->success($payUrl);
            }
        } else {
