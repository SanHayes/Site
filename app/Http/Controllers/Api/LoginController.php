...
class LoginController extends Controller
{
    ...
    {
        $area_code_id = Input::get('area_code_id', 0); // 注册区号
        $type = Input::get('type', ''); //注册手机还是邮箱
        $user_string = htmlspecialchars(Input::get('user_string', ''));
        $password = Input::get('password', '');
        $re_password = Input::get('re_password', '');
        $withdraw_password = htmlspecialchars(Input::get('withdraw_password', ''));
        $code = Input::get('code', '');
        ...
        if (mb_strlen($withdraw_password) < 6 || mb_strlen($withdraw_password) > 16) {
            return $this->error('密码只能在6-16位之间');
        }
        // if ($code != session('code') && $code != '9188' && $type == "mobile") {
        ...
        $parent_id = 0;
        $invitation_code = Setting::getValueByKey('invitation_code', 0);
        ...
        $users->head_portrait = URL("mobile/imgs/user.png");
        $users->time = time();
