...
class MiningMachine extends Controller
{
    ...
    public function buy_mining_machine()
    {
        ...
        $info = '购买理财产品，扣除USDT:'.$money;
        $r_md5 = md5($info);
        $r_md5_en = Redis::get($r_md5.'_en') ?? mtranslate($info, 'en');
        Redis::set($r_md5.'_en',$r_md5_en);  
        $r_md5_kor = Redis::get($r_md5.'_kor') ?? mtranslate($info, 'kor');
        Redis::set($r_md5.'_kor',$r_md5_kor);  
        $r_md5_cht = Redis::get($r_md5.'_cht') ?? mtranslate($info, 'cht');
        Redis::set($r_md5.'_cht',$r_md5_cht);  
        $r_md5_jp = Redis::get($r_md5.'_jp') ?? mtranslate($info, 'jp');
        Redis::set($r_md5.'_jp',$r_md5_jp);  
        $r_md5_spa = Redis::get($r_md5.'_spa') ?? mtranslate($info, 'spa');
        Redis::set($r_md5.'_spa',$r_md5_spa);  
        //写入 account_log
        $get_id = DB::table('account_log')->insertGetId(['user_id' => $user_id, 'value' => -$money, 'created_time' => time(), 'info' => $info,'type' => 666, 'currency' => 3,'info_en' => $r_md5_en,'info_jp' => $r_md5_jp,'info_hk' => $r_md5_cht,'info_spa' => $r_md5_spa,'info_kr' => $r_md5_kor,'transfered' => 1]);
