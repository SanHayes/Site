...
$(document).ready(function () {
    $.get("/order_voice_list", {}, function (res) {
        var str = '';
        var num = '';
        if (res.type === 'ok') {
            var recharge = res.message.recharge;
            if(recharge.length > 0){
                num += recharge.length;
                for (var i = 0; i < recharge.length; i++) {
                    str += '<div class="winui-message-item"><h2>入款订单</h2><div class="content">会员：'+ recharge[i].account_number +' 提交入款，请及时查收！</div></div>';
                }
                var audio = document.createElement("audio");
                audio.src = 'recharge.wav';
                audio.play();
            }
            var withdraw = res.message.withdraw;
            if(withdraw.length > 0){
                num += withdraw.length;
                for (var i = 0; i < withdraw.length; i++) {
                    str += '<div class="winui-message-item"><h2>出款订单</h2><div class="content">会员：'+ withdraw[i].account_number +' 提交出款，请及时受理！</div></div>';
                }
                var audio = document.createElement("audio");
                audio.src = 'withdraw.wav';
                audio.play();
            }
            var orders = res.message.orders;
            if(orders.length > 0){
                num += orders.length;
                for (var i = 0; i < orders.length; i++) {
                    str += '<div class="winui-message-item"><h2>交易订单</h2><div class="content">会员：'+ orders[i].account_number +' 提交订单，请及时处理！</div></div>';
                }
                var audio = document.createElement("audio");
                audio.src = 'orders.wav';
                audio.play();
            }
        }
        $('#num').text(num);
